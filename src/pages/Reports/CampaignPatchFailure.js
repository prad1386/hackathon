...
import { getCampaignPatchFailureReports } from "@store/reporting.duck";
import { constants } from "@constants";
import CSVExport from "@utils/CSVDownload";
import { getIndividualCampaignPatchFailure } from "@store/reporting.duck";
import { errorGlobal } from "@store/notifications.duck";

const CampaignPatchFailurePage = () => {
  const { campaignPatchFailureReport, loading } = useSelector(
  ...
  const downloadCSV = (e, id, report) => {
    SetDownloadData([]);
    dispatch(getIndividualCampaignPatchFailure(id))
      .unwrap()
      .then((res) => {
        if (res.length > 0) {
          SetDownloadData(res);
  ...
  return (
    <Container fluid className="page-container">
      <div className="panel-heading">
        {constants.TEXT_CAMPAIGN_PATCH_REPORT}
      </div>
      <div className="content-layout">
        <div className="sub-heading">
          {constants.TEXT_LIST_OF} {constants.TEXT_CAMPAIGN_PATCH_REPORT}
        </div>
        ...
        <Table
          COLUMNS={COLUMNS}
          DATA={campaignPatchFailureReport}
          isLoading={loading}
          ref={tableRef}
        />
...
export default CampaignPatchFailurePage;
